...
class FeatureEngineer:
    ...
    def calcola_forma_recente(self, squadra, data_partita, n_partite=5):
        ...
        partite_precedenti = self.df[
            (self.df["Date"] < data_partita) & ((self.df["HomeTeam"] == squadra) | (self.df["AwayTeam"] == squadra))
        ].tail(n_partite)
    ...
    def calcola_performance_casa_trasferta(self, squadra, data_partita, casa=True, n_partite=10):
        ...
        if casa:
            partite = self.df[(self.df["Date"] < data_partita) & (self.df["HomeTeam"] == squadra)].tail(n_partite)
        else:
            partite = self.df[(self.df["Date"] < data_partita) & (self.df["AwayTeam"] == squadra)].tail(n_partite)
    ...
    def crea_features(self, min_partite_storiche=10):
        ...
        for idx, row in self.df.iterrows():
            ...
            perf_casa_home = self.calcola_performance_casa_trasferta(
                squadra_casa, data_partita, casa=True, n_partite=10
            )
            perf_trasferta_away = self.calcola_performance_casa_trasferta(
                squadra_trasferta, data_partita, casa=False, n_partite=10
            )
    ...
    print(
        df_features[
            [
                "HomeTeam",
                "AwayTeam",
                "casa_forma_media_punti",
                "trasferta_forma_media_punti",
                "differenza_forma_punti",
                "FTR",
            ]
        ].head(10)
    )
