...
class TestMLPredictions(unittest.TestCase):
    ...
    def test_squadre_serie_a(self):
        ...
        for team in expected_teams:
            self.assertIn(team, available, f"Squadra {team} non disponibile nel calculator")
    ...
    def test_prediction_output_format(self):
        ...
        if "Inter" in self.calculator.squadre_disponibili and "Milan" in self.calculator.squadre_disponibili:

            predizione, probabilita, confidenza = self.calculator.predici_partita_deterministica("Inter", "Milan")
    ...
    def test_probability_sum_equals_one(self):
        ...
        for i in range(len(squadre) - 1):
            ...
            _, probabilita, _ = self.calculator.predici_partita_deterministica(home, away)

            total = sum(probabilita.values())
            self.assertAlmostEqual(total, 1.0, places=5, msg=f"Prob sum for {home} vs {away}: {total}")
    ...
    def test_home_advantage(self):
        ...
        if "Inter" in self.calculator.squadre_disponibili and "Verona" in self.calculator.squadre_disponibili:

            # Inter in casa vs Verona
            pred_home, prob_home, _ = self.calculator.predici_partita_deterministica("Inter", "Verona")

            # Verona in casa vs Inter
            pred_away, prob_away, _ = self.calculator.predici_partita_deterministica("Verona", "Inter")
