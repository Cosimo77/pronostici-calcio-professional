...
class BackgroundAutomation:
    ...
    def _automation_loop(self):
        ...
        while self.running:
            try:
                ...
                logger.info(
                    f"🧠 Weekly retrain check: {should_retrain} (last_retrain: {self.last_retrain}, weekday: {now.weekday()})"
                )
                if should_retrain:
    ...
    def _should_run_daily_update(self, now):
        ...
        if self.last_update:
            ...
            last_update_aware = (
                self.last_update if self.last_update.tzinfo else self.last_update.replace(tzinfo=timezone.utc)
            )
        ...
        return dt_time(5, 0) <= current_time <= dt_time(7, 0) and (
    ...
    def _should_run_weekly_retrain(self, now):
        ...
        if self.last_retrain:
            ...
            last_retrain_aware = (
                self.last_retrain if self.last_retrain.tzinfo else self.last_retrain.replace(tzinfo=timezone.utc)
            )
