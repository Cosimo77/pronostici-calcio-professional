...
class DiarioStorage:
    ...
    def check_duplicate(partita: str, mercato: str) -> Optional[Dict]:
        ...
        # CSV fallback
        df = pd.read_csv(DiarioStorage.CSV_FILE)
        duplicates = df[(df["Partita"] == partita) & (df["Mercato"] == mercato) & (df["Risultato"] == "PENDING")]

        if len(duplicates) > 0:
            row = duplicates.iloc[0]
            return {"quota": float(row["Quota_Sisal"]), "stake": str(row["Stake"])}
    ...
    def create_multipla(multipla_data: Dict, eventi: List[Dict]) -> str:
        ...
        try:
            ...
            logger.info(
                "Multipla creata con successo (DB)",
                group_id=group_id,
        ...
        logger.info(
            "Multipla creata con successo (CSV)",
            group_id=group_id,
            num_eventi=num_eventi,
            tipo=tipo_multipla,
        )
    ...
    def update_evento_multipla(bet_id: int, risultato: str) -> float:
        ...
        try:
            ...
            logger.info(
                "Evento multipla aggiornato (DB)",
                bet_id=bet_id,
                group_id=group_id,
                risultato=risultato,
                profit_finale=profit,
            )
            ...
            logger.error(
                "Errore update evento multipla DB, fallback to CSV",
                bet_id=bet_id,
                error=str(e),
            )
            # Fall through to CSV
        ...
        logger.info(
            "Evento multipla aggiornato (CSV)",
            bet_id=bet_id,
            risultato=risultato,
            profit_finale=profit,
        )
    ...
    def delete_multipla(group_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                "Errore delete multipla DB, fallback to CSV",
                group_id=group_id,
                error=str(e),
            )
            # Fall through to CSV
