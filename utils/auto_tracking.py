...
class AutoTracker:
    ...
    def _ensure_file_exists(self):
        ...
        if not os.path.exists(self.tracking_file):
            df = pd.DataFrame(columns=[
                'Data', 'Giornata', 'Casa', 'Ospite', 'Mercato',
                'Predizione', 'Probabilita_Sistema', 'Quota', 'EV_%',
                'Risultato_Reale', 'Corretto', 'Profit', 'Note'
            ])
    ...
    def track_prediction(
        ...
        try:
            ...
            logger.info(
                f"✅ Predizione tracciata: {casa} vs {ospite} | "
                f"{mercato}: {predizione} ({probabilita:.1%})"
            )
            return True
    ...
    def update_result(
        ...
        try:
            ...
            for idx in df[mask].index:
                ...
                # Correttezza
                corretto = self._check_correctness(predizione, risultato_reale, df.loc[idx, 'Mercato'])
    ...
    def _check_correctness(self, predizione: str, risultato: str, mercato: str) -> bool:
        ...
        if mercato == '1X2':
            mapping = {
                'casa': 'h', 'home': 'h', '1': 'h',
                'pareggio': 'd', 'draw': 'd', 'x': 'd',
                'away': 'a', 'trasferta': 'a', '2': 'a'
            }
